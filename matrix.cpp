...
namespace Matrix{
    ...
    SquareMat SquareMat::operator- (){
        // copies the matrix
        SquareMat copy = SquareMat(*this);

        // multiplies copy by -1 and returns it
        return copy*(-1);
    }
    
    SquareMat SquareMat::operator~ (){
        // creates a matrix of the same size
        SquareMat copy = SquareMat(this->size());

        // copies the values from this matrix to copy such that value i,j in this matrix is inserted at j,i in copy
        for(int i=0; i<this->n; i++){
            for(int j=0; j<this->n; j++){
                // copies value over
                copy[j][i] = (*this)[i][j];
